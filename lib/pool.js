...
let knownProxies = JSON.parse(fs.readFileSync('allowed_proxies.json'));
...
if (cluster.isMaster) {
    ...
    fs.watch('allowed_proxies.json', function(eventType, filename){
        if (eventType === 'changed'){
            fs.readFile(filename, function (err, data) {
                if(!err){
                    knownProxies = JSON.parse(data);
                    sendToWorkers({message: 'newProxies', data: knownProxies});
                }
            });
        }
    });
...
function sendToWorkers(data) {
    for (let id in cluster.workers) {
        if (cluster.workers.hasOwnProperty(id)){
            ...
            if (!worker.send(data)) {
                console.log(threadName + 'Worker is not responding to data push, killing');
                worker.kill();
            }
        }
    }
...
function retargetMiners() {
    console.log(threadName + "Performing difficulty check on miners");
    ...
    for (let minerId in activeMiners) {
        if (activeMiners.hasOwnProperty(minerId)) {
            ...
            if (Date.now() - miner.lastContact > global.config.pool.minerTimeout * 1000) {
                ...
                activeMiners[minerId].socket.end();  // End the socket, then delete the ref.
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, socket) {
    ...
    if (agent && agent.includes('xmr-stak')){
        this.isAsync = false;
    }
    this.socket = socket;
    this.paymentID = null;
    ...
    async.waterfall([
        function(callback){
            if (global.config.pool.trustedMiners && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 &&
                ...
                shareType = true;
                return callback(null, Buffer.from(resultHash, 'hex'));
            }
            else {
                convertedBlob = global.coinFuncs.convertBlob(shareBuffer);
                shareType = false;
                if (miner.isAsync) {
                    global.coinFuncs.hashAsync(convertedBlob, function (err, result) {
                        return callback(null, result);
                    });
                } else {
                    return callback(null, global.coinFuncs.hashSync(convertedBlob));
                }
            }
        },
...
function handleMinerData(method, params, ip, portData, sendReply, pushMessage, socket) {
    ...
    switch (method) {
        case 'login':
            ...
            let minerId = uuidV4();
            miner = new Miner(minerId, params.login, params.pass, ip, difficulty, pushMessage, 1, portData.portType, portData.port, params.agent, socket);
        ...
        case 'submit':
            ...
            processShare(miner, job, blockTemplate, params.nonce, params.result, sendReply);
            break;
        case 'keepalived':
            if (!miner) {
                sendReply('Unauthenticated');
...
if (cluster.isMaster) {
    ...
    cluster.on('online', function (worker) {
        ...
        worker.send({message: 'newProxies', data: knownProxies});
    });
    ...
    async.each(global.config.ports, function (portData) {
        ...
        let handleMessage = function (socket, jsonData, pushMessage) {
            ...
            handleMinerData(jsonData.method, jsonData.params, socket.remoteAddress, portData, sendReply, pushMessage, socket);
        };
        ...
        function socketConn(socket) {
            ...
            socket.on('data', function (d) {
                dataBuffer += d;
                let packetSize = Buffer.byteLength(dataBuffer, 'utf8');
                if (packetSize > 10240 && !(socket.remoteAddress in knownProxies)) { //10KB
                    dataBuffer = null;
                    console.warn(threadName + 'Excessive packet size from: ' + socket.remoteAddress);
                    socket.destroy();
                    return;
                } else if (packetSize > 10240 && socket.remoteAddress in knownProxies){
                    console.warn(threadName + `Permitting ${socket.remoteAddress} extra size as they are a known proxy.  Packet size: ${packetSize}`);
                }
